...
# 保存用のボタン表示名テキストファイル
button_names_file = Path("PathIDButtonList.txt")
# バージョン情報
Version = 2.0
...
APP_FOLDER = Path("TyAppsLauncher")
...
def check_update():
    ...
    try:
        ...
        if latest_version > Version:
            print(f"新しいバージョン {latest_version} が利用可能です。アップデートを開始します。")
            update_app()
        else:
            print("最新のバージョンを使用しています。")
...
def apply_update(update_file):
    ...
    try:
        ...
        print("アップデートが完了しました。アプリを再起動します。")
        restart_app()
    except Exception as e:
        print(f"アップデート適用中にエラーが発生しました: {e}")

def restart_app():
    """アプリを再起動"""
    print("アプリを再起動します...")
    python = sys.executable
    os.execl(python, python, *sys.argv)  # 現在のプロセスを新しいプロセスに置き換え
...
label = tk.Label(root, text="Create new appspath_X.txt in Typath folder after the button clicking")
label.place(relx=0.5, rely=0.1, anchor=tk.CENTER)

# **別ウィンドウにボタンを表示**
rootf = tk.Toplevel(root)
rootf.title("TyAppsLauncher@V" + str(Version))
...
for file_num, button_label in enumerate(button_labels, start=1):
    file_path = Path("Typath") / f"appspath_{file_num}.txt"
    function_dict[button_label] = partial(execute_file, file_path)
    create_button_in_window(button_label)
